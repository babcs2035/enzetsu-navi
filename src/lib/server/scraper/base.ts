...
export abstract class BaseScraper {
  ...
  protected async saveSpeech(data: SpeechData) {
    try {
      ...
      const candidate = await this.getOrCreateCandidate(data.candidate_name, party.id)
      ...
      console.log(`演説データを保存: ${data.candidate_name} - ${data.location_name}`)
      return speech
    } catch (error) {
  ...
  async run(): Promise<number> {
    ...
    try {
      ...
      console.log(`スクレイピング完了: ${this.partyName} - ${savedCount}件保存`)
      return savedCount
    } catch (error) {
